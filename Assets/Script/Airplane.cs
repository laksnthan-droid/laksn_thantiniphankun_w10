...
public class AirplanePhysicsController : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        Vector3 sideVel = Vector3.Dot(rb.velocity, transform.right) * transform.right;
        ...
        // --- TORQUE CONTROL --------
        rb.AddRelativeTorque(new Vector3(pitch * pitchPower, yaw * yawPower, -roll * rollPower), ForceMode.Acceleration);

        // -------- BANKED TURN --------
        float bankAmount = Vector3.Dot(transform.right, Vector3.up);
        rb.AddForce(transform.right * bankAmount * forwardSpeed * turnStrength, ForceMode.Acceleration);
